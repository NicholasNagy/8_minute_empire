...
int main()
{
	...
	int countryBlockSize = 180;
	int grid_width = 30;
	int grid_height = 29;

	// + 1 so that the last grid lines fit in the screen.
	int window_width = 1800 + 1;
	int window_height = 900 + 1;
	...
	while (!quit) {
		...
		for (int x = 0; x <= (30 * grid_cell_size); x += grid_cell_size*5)
		{
		    SDL_RenderDrawLine(renderer, x, 0, x, window_height);
		}

		for (int y = 0; y < (25 * grid_cell_size); y += grid_cell_size*5) 
		{
